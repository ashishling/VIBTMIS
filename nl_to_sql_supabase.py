...
Uses OpenAI API to convert natural language questions to SQL queries for Supabase PostgreSQL.

Usage:
    python nl_to_sql_supabase.py "What are the top 10 stores by revenue in 2024?"
    python nl_to_sql_supabase.py "Show me monthly transaction trends"
"""
...
- value (DOUBLE): The metric value for that store/month/parameter combination
...
Instructions:
1. Generate PostgreSQL-compatible SQL queries (not DuckDB syntax)
...
def execute_sql_query(sql_query: str) -> str:
    """Execute SQL query using Supabase and return results."""
    try:
        ...
        output = supabase_execute_query(sql_with_limit)
        ...
        if not has_limit:
            try:
                ...
                if 'FROM mis_long' in sql_query.upper() and 'GROUP BY' not in sql_query.upper():
                    ...
                    count_output = supabase_execute_query(count_query)
...
def main():
    ...
    try:
        ...
        if args.interactive:
            print("🤖 BT Store Analytics - Interactive SQL Generator")
